                ...
                for metric_name, value in resource_usage.items():
                    gauge_name = (
                        "git,operation={},repo={},env={},metric={}".format(
                            command, repository, environment, metric_name
                        )
                    )
...
class PackBackendProtocol(PackServerProtocol):
    ...
    def requestReceived(self, command, raw_pathname, params):
        ...
        if command == b"turnip-create-repo":
            try:
                ...
                self.die(
                    b"Could not create repository: %s"
    ...
    def spawnGit(
        ...
        cmd = os.path.join(os.path.dirname(__file__), "git_helper.py").encode(
            "UTF-8"
        )
        args = [cmd]
    ...
    def _createRepo(self, pathname, clone_from, auth_params):
        ...
        try:
            ...
            yield proxy.callRemote(
                "abortRepoCreation", six.ensure_text(pathname), auth_params
            ).addTimeout(xmlrpc_timeout, default_reactor)
...
class PackBackendFactory(protocol.Factory):
    ...
    def __init__(
        self, root, hookrpc_handler=None, hookrpc_sock=None, statsd_client=None
    ):
        self.root = root
    ...
    def requestReceived(self, command, pathname, params):
        ...
        try:
            ...
            translated = yield proxy.callRemote(
                "translatePath",
                six.ensure_text(pathname),
                permission,
                auth_params,
            ).addTimeout(self.factory.virtinfo_timeout, self.factory.reactor)
            self.log.info(
                "Translation result: {translated}", translated=translated
            )
            if "trailing" in translated and translated["trailing"]:
                self.die(
                    VIRT_ERROR_PREFIX + b"NOT_FOUND Repository does not exist."
                )
            ...
            fault_type = translate_xmlrpc_fault(e.faultCode).name.encode(
                "UTF-8"
            )
