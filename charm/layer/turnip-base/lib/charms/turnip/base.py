...
def install_payload():
    ...
    try:
        if config["swift_container_name"]:
            ...
            swift_fetch(
                os.path.join("turnip-builds", object_name),
                archive_path,
...
def configure_rsync():
    ...
    if config["log_hosts_allow"]:
        templating.render(
            "turnip-rsync.j2",
            "/etc/rsync-juju.d/010-turnip.conf",
            config,
            perms=0o644,
        )
...
def publish_website(website, name, port):
    ...
    try:
        service_options = yaml.safe_load(config.get("haproxy_service_options"))
    except Exception:
